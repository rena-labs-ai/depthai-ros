...
StereoParamHandler::StereoParamHandler(std::shared_ptr<rclcpp::Node> node, const std::string& name, const std::string& deviceName, bool rsCompat)
    ...
    // The rectified pair <side>_rect. Its projection keeps the whole lens: the
    // driver uploads the rectification mesh (Stereo::uploadRectifyMesh), so
    // this is ~129 deg on an OAK-W rather than the ~97 deg the device's own
    // left-K recipe would give.
...
void StereoParamHandler::declareParams(std::shared_ptr<dai::node::StereoDepth> stereo) {
    ...
    // Hard-guarantees i_stereo_conf_threshold on the published depth (zeroes depth
    // where the confidence map is below it), regardless of upstream post-filters
    // re-adding sub-threshold pixels. Same scale as i_stereo_conf_threshold.
